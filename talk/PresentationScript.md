...
Everything we do every day is analysis.  OIAs, information requests, papers, even our own team projects like investigating the integrity of BO Universes; these can all be done in a reproducible way.
...
If you've have used email today, you've already done reproducible analysis.

# Text Files
(email)

This is an email as it appears to a computer.  At first glance it looks like gobbledegook, but we're all smart, technical people, and with a bit of effort we can still read it.  There's the date, there's who it was sent from, and my email address.  There's who it was sent to, and Harold's email address.  Here's the content of the email---I've sent my brother a hilarious Excel joke, and a link to a funny website.
...
The pace of development of R is exploding.  That's why businesses like ANZ use it.  Because it's open source, every one will benefit from everybody-else's investment in it, so if ANZ develops a new feature in R, everyone else gets to use it too.  Development is so fast, that there's almost nothing in R that hasn't been updated in the last couple of years.

This graph is actually slightly misleading, and I can use that to illustrate several great things about R.  I've adapted this graph from its original publication on someone's blog.  The change I made was to label the x-axis "Year of Last Update".  Without that label, you might think that the number of packages available was increasing exponentially.  That might be true, but the graph doesn't show that.  However, the blog where this graph was published interpreted it that way.  Fortunately, because they made their code available to, as a text file, I was able to work out exactly what was going on and correct it.